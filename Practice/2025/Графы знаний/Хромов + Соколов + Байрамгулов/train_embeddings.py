...
def train_graph_embeddings(ttl_file_path: str,
                           save_dir: str = "brawl_embedding_results"):
    """
    Обучение TransE-эмбеддингов для RDF-графа.
    ttl_file_path — путь к Turtle-файлу (brawl_graph.ttl по умолчанию).
...
def load_saved_model(directory: str = "brawl_embedding_results"):
    """
...
def plot_embeddings_2d(points_2d,
    ...
    def _shorten(uri: str) -> str:
        bases = {
            "http://example.org/wowkg#": "wow:",
            "http://example.org/brawlstars#": "bs:",
        }
...
def visualize_embeddings(model: TransE,
                         ...
                         method: str = "tsne",
                         max_labels: int = 75,
                         save_path: str = "brawl_embedding_results/embeddings.png",
                         perplexity: int = 30,
                         max_points: int = 1200,
                         random_state: int = 42):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Обучение и визуализация эмбеддингов KG "
                    "(PyKEEN + TransE, по умолчанию для Brawl Stars)"
    )
    parser.add_argument(
        "--ttl",
        default="brawl_graph.ttl",
        help="Путь к Turtle-файлу с графом (по умолчанию brawl_graph.ttl)",
    )
    parser.add_argument(
        "--results-dir",
        default="brawl_embedding_results",
    ...
    parser.add_argument(
        "--entity",
        default="http://example.org/brawlstars#Amber",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[
            "http://example.org/brawlstars#Amber",
            "http://example.org/brawlstars#Bea",
        ],
